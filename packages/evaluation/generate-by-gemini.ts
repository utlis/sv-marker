...
import { generateSentenceStructureDataByGemini } from "./generate-sentence-structure-data-by-gemini.js";

const args = process.argv.slice(2);
const modelName = args
  .find((arg) => arg.startsWith("--model="))
  ?.split("=")[1] as GeminiModelName | undefined;
if (!modelName || !geminiModelNames.includes(modelName))
...
const runConfigurations: RunConfiguration[] = geminiTemperatureValues.flatMap(
  (temperature) =>
    geminiThinkingLevelValues.flatMap((thinkingLevel) => {
...
await Promise.all(
  runConfigurations.map(
    async ({ modelName, temperature, thinkingLevel, runIndex }) => {
      ...
      for (const dataset of datasets) {
        while (true) {
          ...
          const result = await generateSentenceStructureDataByGemini(
            dataset.englishText,
            {
              modelName,
              temperature,
              seed: runIndex * 100 + retryIndex,
              thinkingLevel,
            },
          );
          ...
          if (result.success) {
            ...
            console.log(
              `[model: ${modelName}, temperature: ${temperature}, thinkingLevel: ${thinkingLevel}, run index: ${runIndex}] Processed dataset ID ${dataset.id} successfully.`,
            );
          } else {
            const existingLog = logs.find((log) => log.id === dataset.id);
            if (existingLog) {
            ...
            console.log(
              `[model: ${modelName}, temperature: ${temperature}, thinkingLevel: ${thinkingLevel}, run index: ${runIndex}] Failed to process dataset ID ${dataset.id}. Retry index: ${retryIndex}.`,
            );
          }
